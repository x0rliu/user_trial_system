...
"""
    ...
    from app.db.historical import (
        delete_insights_by_context_and_type_with_cursor,
        insert_historical_insight_run_with_cursor,
        insert_historical_trial_insight_with_cursor,
    )

    # -------------------------
    # Persist as one DB unit
    # -------------------------
    conn = get_db_connection()
    cursor = conn.cursor()

    try:
        # Clear previous AI output before replacing it. Summary and insights
        # are both generated by this flow, so they must be refreshed together.
        delete_insights_by_context_and_type_with_cursor(
            cursor,
            context_id,
            "ai_summary",
        ...
        insight_run_id = insert_historical_insight_run_with_cursor(
            cursor,
            context_id=context_id,
            trigger_type="manual_ai",
            generation_version="ai_v2",
        )
        ...
        for item in insights:
            ...
            # -------------------------
                ...
                insert_historical_trial_insight_with_cursor(
                    ...
                    insight_summary=raw_title.strip(),
                    insight_json=json.dumps(item),
                    source_sample_size=sample_size,
                    filters_applied=None,
                )
