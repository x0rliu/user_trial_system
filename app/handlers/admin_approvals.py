...
def render_product_trial_approval_block(items: list[dict]) -> str:
    ...
    for a in items:
        ...
        approval_id = round_id

        rows.append(f"""
        <!-- Primary row -->
        <tr class="approval-row" data-approval-id="{approval_id}">
            <td>
                <a href="/admin/approvals/project?project_id={p.get('ProjectID')}">
                    {p.get("ProjectName")}
                </a>
            </td>
            <td>{p.get("requested_by_name") or "—"}</td>
            <td>{p.get("ProductType")}</td>
            <td>{p.get("submitted_at") or "—"}</td>
            <td>
        ...
        <tr class="approval-detail hidden" data-approval-id="{approval_id}">
            <td colspan="5">
                <div class="approval-form-wrapper">
                    <form method="post" action="/admin/approvals/submit">
                        ...
                        <input type="hidden" name="requested_by" value="{p.get('requested_by_user_id')}">
...
def render_bonus_survey_approval_block(items: list[dict]) -> str:
    ...
    for a in items:
        p = a["payload"]
        approval_id = a["approval_id"]  # tracker_id

        rows.append(f"""
        <tr class="approval-row" data-approval-id="{approval_id}">
            <td>
                <a href="/admin/approvals/view?tracker_id={approval_id}">
                    {p.get("survey_title")}
                </a>
            </td>
            <td>{p.get("requested_by_name") or "—"}</td>
            <td>
                <a href="/surveys/bonus/edit?survey_id={p.get("bonus_survey_id")}">
                    Open
                </a>
            </td>
            <td>{p.get("submitted_at") or "—"}</td>
